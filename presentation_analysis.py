...
# Import from Update.py
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from Update import (SimulationConfig, MARKET_DATA_FILE, YEARS, SCENARIOS, 
                   load_market_data, BessOptimizerQuadratic, PRICE_TAKER_CAPACITY,
                   MAIN_CAPACITIES, DEFAULT_SLOPE)
...
def load_base_results():
    ...
    if os.path.exists(summary_path_new):
        print(f"Lade Ergebnisse aus {summary_path_new}...")
        df = pd.read_csv(summary_path_new)
        return df
    elif os.path.exists(summary_path_old):
        print(f"Lade Ergebnisse aus {summary_path_old} (alte Version ohne _pt Szenarien)...")
        df = pd.read_csv(summary_path_old)
        return df
    else:
        print("Keine Ergebnisse gefunden. Bitte erst Update.py ausführen!")
        return None
...
def plot_arbitrage_per_mwh(results_df):
    ...
    # Farben: von dunkel (klein) zu hell (groß)
...
# ==========================================
# 5. PLOT: Price-Taker vs Price-Maker Comparison
# ==========================================
def plot_price_taker_comparison(results_df):
    """Plot 4: Compare endogenous (price impact) vs price-taker benchmark"""
...
# ==========================================
# 5b. PLOT: Price Impact Cost Analysis
# ==========================================
def plot_price_impact_cost(results_df):
    """
    Plot 5b: Compare revenue WITH price impact (α>0) vs WITHOUT price impact (α=0)
    
    Shows the "cost of market power" - how much revenue is lost due to price impact.
    Uses data from CSV where scenarios like '10' and '10_pt' are both available.
    """
    ...
    if len(capacities) == 0:
        print("  Warning: No matching scenarios found. Run Update.py first to generate _pt scenarios.")
        return None
...
def plot_yearly_comparison():
    ...
    # Filter: nur Hauptszenarien (ohne _pt Suffix)
...
def plot_sensitivity_matrix_table(pct_changes):
    """Plot: Sensitivitäts-Matrix als Tabelle"""
...
def analyze_spread_evolution():
    """
    Analysiert die Evolution der Preis-Spreads über die Jahre:
    - Peak/Off-Peak Spread
    - Tägliche Min/Max Spreads
    - Volatilität
    """
    print("\nAnalysiere Spread-Evolution 2019-2024...")
    ...
    for year in YEARS:
        ...
        # Stündliche Analyse (Peak = 8-20 Uhr, Off-Peak = Rest)
        ...
        stats = {
            ...
            'extreme_spread_days': (daily_spreads > 100).sum(),  # Tage mit >100€ Spread
        }
...
def plot_spread_evolution():
    """Plot: Daily Intraday Spread Boxplot (akademischer Standard)"""
    print("\nAnalysiere tägliche Spreads...")
    
    df = load_market_data()
    
    # Berechne tägliche Spreads (Max - Min) für jeden Tag
...
# ==========================================
# 11. PLOT: Revenue Change Rate Comparison (2019 vs 2024)
# ==========================================
def plot_revenue_change_comparison_2019_2024():
    """Vergleicht die Veränderungsrate des spezifischen Erlöses zwischen 2019 und 2024."""
    print("\nVergleiche Erlös-Veränderungsraten für 2019 vs. 2024...")
    ...
    for year in tqdm(years_to_compare, desc="Calculating 2019 & 2024"):
        year_df = df[df.index.year == year]
        if len(year_df) == 0:
            print(f"Warnung: Keine Daten für das Jahr {year} gefunden.")
            continue
...
if __name__ == "__main__":
    ...
    if results_df is not None:
        ...
        print("ERROR: No results found. Please run Update.py first!")
